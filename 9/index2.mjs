...
const answer = histories.reduce((sum, history) => {
    ...
    currHistories[currHistories.length - 1].addWithSameDifference(false);

    for (let i = currHistories.length - 2; i >= 0; i--) {
        const difference = (() => {
            const { numbers: nextHistoryNumbers } = currHistories[i + 1];
            return nextHistoryNumbers[0];
        })();

        const currHistory = currHistories[i];
        currHistory.add(currHistory.numbers[0] - difference, false);
    }

    return sum + currHistories[0].numbers[0];
}, 0);
